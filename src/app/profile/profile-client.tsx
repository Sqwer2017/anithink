...
import { Camera, Clock3, Eye, EyeOff, Film, Heart, History, Save, Send, MessageCircle, Gamepad2, ExternalLink, ChevronDown, ChevronUp } from "lucide-react";
...
function readCompletedIds() {
  try { const value: unknown = JSON.parse(window.localStorage.getItem(WATCH_STATUS_STORAGE_KEY) ?? "{}"); return value && typeof value === "object" && !Array.isArray(value) ? Object.entries(value).filter(([, status]) => status === "completed").map(([id]) => id) : []; } catch { return []; }
}
...
export function ProfileClient() {
  ...
  useEffect(() => {
    const loadAnime = (ids: string[], set: (anime: Anime[]) => void) => {
      ...
      void fetch(`/api/saved?ids=${ids.slice(0, 30).join(",")}`).then((response) => response.json()).then(set).catch(() => set([]));
    };
    const sync = () => {
      const saved = readProfile();
      setProfile(saved); setHidden(saved.isSavedPrivate);
    ...
    sync();
    window.addEventListener("anithink:watch-status-changed", sync);
    return () => window.removeEventListener("anithink:watch-status-changed", sync);
  }, []);
  ...
  // Недавно посещаемые ограничены строго 6 карточками
  ...
  return <>
    <section className="overflow-hidden rounded-3xl border border-border/70 bg-card shadow-cyber">
      <label className="relative block h-44 cursor-pointer overflow-hidden bg-[radial-gradient(circle_at_20%_30%,rgb(var(--accent)/0.45),transparent_25%),linear-gradient(120deg,rgb(var(--bg-panel-2)),rgb(var(--bg-main)))]">
        <input type="file" accept="image/*" className="hidden" onChange={(event) => void upload("cover", event.target.files?.[0])} />
      ...
      <div className="relative p-5 md:p-7">
        <label className="absolute -top-14 h-28 w-28 cursor-pointer overflow-hidden rounded-3xl border-2 border-accent bg-accent-gradient shadow-neon">
          <input type="file" accept="image/*" className="hidden" onChange={(event) => void upload("avatar", event.target.files?.[0])} />
          {profile.avatar ? <img src={profile.avatar} alt="" className="h-full w-full object-cover" /> : <span className="flex h-full items-center justify-center text-3xl font-bold text-background">{profile.nickname[0]?.toUpperCase()}</span>}
        </label>
        <div className="ml-32 flex items-start justify-between gap-3">
          ...
          <div className="hidden sm:block"><CharacterEditor profile={profile} onChange={updateCharacter} /></div>
        </div>
        <div className="mt-5 flex justify-end sm:hidden"><CharacterEditor profile={profile} onChange={updateCharacter} /></div>
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <input value={profile.nickname} onChange={(event) => setProfile({ ...profile, nickname: event.target.value })} className="rounded-xl border border-border bg-surface px-4 py-3 text-sm" placeholder="Ник" />
          <input value={profile.tag} onChange={(event) => setProfile({ ...profile, tag: event.target.value.replace(/^@/, "") })} className="rounded-xl border border-border bg-surface px-4 py-3 text-sm" placeholder="Тег" />
        </div>
        <textarea value={profile.bio} onChange={(event) => setProfile({ ...profile, bio: event.target.value.slice(0, 300) })} className="mt-3 h-24 w-full resize-none rounded-xl border border-border bg-surface px-4 py-3 text-sm" placeholder="BIO" />
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <Social icon={Send} value={profile.telegram} placeholder="Telegram link" onChange={(telegram) => setProfile({ ...profile, telegram })} />
    ...
    {/* Недавно посещаемые (Макс 6) */}
    <Block title="Недавно посещаемые" icon={History} anime={visibleRecent} />

    {/* Просмотрено — с плавной анимацией выката */}
    <section className="mt-7 rounded-3xl border border-border/70 bg-card p-5 shadow-panel">
      ...
      {completed.length ? (
        <>
          ...
          {/* Плавно выдвигающийся блок с карточками свыше 6 */}
          {completed.length > 6 && (
            <>
              <div
    ...
    <section className="mt-7 rounded-3xl border border-border/70 bg-card p-5 shadow-panel">
      <div className="flex items-center justify-between">
        <h2 className="flex items-center gap-2 font-display text-xl font-bold"><Heart className="h-5 w-5 text-accent" />Любимые аниме</h2>
        <button type="button" onClick={() => { const next = !hidden; setHidden(next); persist({ ...profile, isSavedPrivate: next }); }} className="rounded-xl border border-border bg-surface p-2 text-muted hover:text-accent">
          {hidden ? <Eye /> : <EyeOff />}
        </button>
      </div>
      <div className={`mt-4 transition ${hidden ? "pointer-events-none select-none opacity-20 blur-sm" : ""}`}>
        {favorites.length ? (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {favorites.map((anime, index) => <AnimeCard key={anime.id} anime={anime} index={index} />)}
          </div>
        ) : (
...
function CharacterEditor({ profile, onChange }: { profile: LocalProfile; onChange: (index: number, value: string) => void }) { 
  return (
    <div className="flex flex-wrap items-center gap-3">
      <a href="https://shikimori.one/characters" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-accent/40 bg-accent/10 px-2.5 py-1.5 text-[11px] font-semibold text-accent hover:bg-accent/20">
        Найти ID <ExternalLink className="h-3.5 w-3.5" />
      </a>
      {[0, 1, 2].map((index) => (
        <label key={index} className="relative">
          <img src={`https://shikimori.one/system/characters/original/${profile.characterIds[index] || 80}.jpg`} alt="" className="h-14 w-14 rounded-full border-2 border-accent object-cover shadow-neon-sm" />
          <input value={profile.characterIds[index] || ""} onChange={(event) => onChange(index, event.target.value)} className="absolute -bottom-3 left-0 w-14 rounded bg-surface px-1 text-center text-[10px] text-muted" aria-label="ID персонажа Shikimori" />
        </label>
      ))}
    </div>
  ); 
...
function Social({ icon: Icon, value, placeholder, onChange }: { icon: typeof Send; value: string; placeholder: string; onChange: (value: string) => void }) { 
  return (
    <label className="flex items-center gap-2 rounded-xl border border-border bg-surface px-3 text-muted">
      <Icon className="h-4 w-4 text-accent" />
      <input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="min-w-0 flex-1 bg-transparent py-3 text-xs outline-none" />
      {value && <a href={value.startsWith("http") ? value : `https://${value}`} target="_blank" rel="noreferrer" className="text-accent">↗</a>}
    </label>
  ); 
}
...
function Block({ title, icon: Icon, anime }: { title: string; icon: typeof History; anime: Anime[] }) { 
  return (
    <section className="mt-7 rounded-3xl border border-border/70 bg-card p-5 shadow-panel">
      <h2 className="flex items-center gap-2 font-display text-xl font-bold"><Icon className="h-5 w-5 text-accent" />{title}</h2>
      {anime.length ? (
        <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
